...
int CChannelList::show()
{
	...
	while (loop) {
		...
		else if (!empty && (msg == CRCInput::RC_up || (int)msg == g_settings.key_chlist_pageup || (int)msg == g_settings.key_pageup || 
				    msg == CRCInput::RC_down || (int)msg == g_settings.key_chlist_pagedown || (int)msg == g_settings.key_pagedown))
		{
...
void CChannelList::paintItem(int pos, const bool firstpaint)
{
	...
	if (i_selected && g_settings.j00zek_channellist_png_selector) { 
		if (!g_PicViewer->DisplayImage(SELECTOR_PNG, x+2, ypos, (width-SCROLLBAR_WIDTH-4), fheight, 1 /*transparency flag*/)) {
...
// j00zek we don't paint scrambled icons
		...
		if (!(p_event->description.empty()) && g_settings.j00zek_channellist_name_and_descr)
		{
...
/*		snprintf(ChannelID, sizeof(ChannelID), "%04Lu", chan->getChannelID()); */
		...
		char nameAndDescription[64]; //j00zek our vfd can display 64 chars max
		if (g_settings.lcd_info_line == 3)
			snprintf(nameAndDescription, sizeof(nameAndDescription), "%s", p_event->description.c_str());
		else 
		...
		if ((y+ theight+ pig_height + i*ffheight) < (y+ theight+ pig_height + infozone_height - firstRowY + nextRowsY)) //j00zek w trybie epg wyswietlamy pierwsza pozycje przed epg, dopiero reszta jest po
		{
			...
			if (e->eventID)
			{
				...
				if (first)
					...
					g_Font[eventFont]->RenderString(x+ width+ OFFSET_INNER_MID, y+ theight + pig_height + firstRowY - nextRowsY + i*ffheight, startTimeWidth, startTime, color);
			}
			if (first)
				...
				g_Font[eventFont]->RenderString(x+ width+ OFFSET_INNER_MID +startTimeWidth, y+ theight+ pig_height + firstRowY - nextRowsY + i*ffheight, infozone_width - startTimeWidth - 2*OFFSET_INNER_MID, e->description, color);
		}
		else
		{
			break;
